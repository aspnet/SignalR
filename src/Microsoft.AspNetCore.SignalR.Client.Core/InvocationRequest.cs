    ...
    internal abstract class InvocationRequest : IDisposable
    {
        ...
        private class NonStreaming : InvocationRequest
        {
            ...
            public override ValueTask<bool> StreamItem(object item)
            {
                ...
                _completionSource.TrySetException(new InvalidOperationException($"Streaming hub methods must be invoked with the '{nameof(HubConnection)}.{nameof(HubConnection.StreamAsChannelCoreAsync)}' method."));
