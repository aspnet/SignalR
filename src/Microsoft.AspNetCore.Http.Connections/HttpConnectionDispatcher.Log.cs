...
namespace Microsoft.AspNetCore.Http.Connections
{
    public partial class HttpConnectionDispatcher
    {
        private static class Log
        {
            ...
            private static readonly Action<ILogger, HttpTransportTypes, Exception> _transportNotSupported =
                LoggerMessage.Define<HttpTransportTypes>(LogLevel.Debug, new EventId(7, "TransportNotSupported"), "{TransportType} transport not supported by this connection handler.");

            private static readonly Action<ILogger, HttpTransportTypes, HttpTransportTypes, Exception> _cannotChangeTransport =
                LoggerMessage.Define<HttpTransportTypes, HttpTransportTypes>(LogLevel.Error, new EventId(8, "CannotChangeTransport"), "Cannot change transports mid-connection; currently using {TransportType}, requesting {RequestedTransport}.");
            ...
            public static void TransportNotSupported(ILogger logger, HttpTransportTypes transport)
            {
                _transportNotSupported(logger, transport, null);
            }

            public static void CannotChangeTransport(ILogger logger, HttpTransportTypes transport, HttpTransportTypes requestTransport)
            {
