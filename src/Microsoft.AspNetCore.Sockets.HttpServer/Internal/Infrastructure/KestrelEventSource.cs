...
namespace Microsoft.AspNetCore.Sockets.HttpServer
{
    ...
    public sealed class KestrelEventSource : EventSource
    {
        ...
        [NonEvent]
        public void ConnectionStart(ConnectionContext context)
        {
            // avoid allocating strings unless this event source is enabled
            if (IsEnabled())
            {
                var httpConn = context.Features.Get<IHttpConnectionFeature>();
                ConnectionStart(
                    context.ConnectionId,
                    httpConn.LocalIpAddress.ToString() + ":" + httpConn.LocalPort,
                    httpConn.RemoteIpAddress.ToString() + ":" + httpConn.RemotePort);
            }
        }
