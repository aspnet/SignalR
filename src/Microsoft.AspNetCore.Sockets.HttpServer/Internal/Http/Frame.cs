...
namespace Microsoft.AspNetCore.Sockets.HttpServer
{
    public abstract partial class Frame : IFrameControl
    {
        ...
        public ConnectionContext ConnectionInformation => _frameContext.Connection;
        ...
        public string HttpVersion
        {
            get
            {
                if (_httpVersion == HttpVersionEnum.Http11)
                {
                    return "HTTP/1.1";
                }
                if (_httpVersion == HttpVersionEnum.Http10)
                {
                    return "HTTP/1.0";
                }
            ...
            set
            {
                ...
                {
                    _httpVersion = HttpVersionEnum.Http11;
                }
                else if (ReferenceEquals(value, "HTTP/1.0"))
                {
                    _httpVersion = HttpVersionEnum.Http10;
                }
                else
                {
        ...
        private void HttpVersionSetSlow(string value)
        {
            ...
            {
                _httpVersion = HttpVersionEnum.Unknown;
            }
        }
        ...
        private void CreateResponseHeader(bool appCompleted)
        {
            ...
            if (_canHaveBody)
            {
                if (!hasTransferEncoding && !responseHeaders.ContentLength.HasValue)
                {
                    ...
                    else
                    {
                        ...
                        if (_httpVersion == HttpVersionEnum.Http11 && StatusCode != StatusCodes.Status101SwitchingProtocols)
                        {
                            _autoChunk = true;
            ...
            if (!hasConnection)
            {
                ...
                else if (_httpVersion == HttpVersionEnum.Http10)
                {
