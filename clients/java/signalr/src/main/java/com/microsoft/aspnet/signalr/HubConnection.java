...
public class HubConnection {
        ...
        (payload) -> {
          if (!handshakeReceived) {
            ...
            HandshakeResponseMessage handshakeResponse =
                HandshakeProtocol.parseHandshakeResponse(handshakeResponseString);
            ...
            switch (message.getMessageType()) {
              case INVOCATION:
                ...
                if (handlers.containsKey(invocationMessage.target)) {
                  ...
                  logger.log(
                      LogLevel.Warning,
                      "Failed to find handler for %s method",
                      invocationMessage.target);
                }
                break;
              case CLOSE:
              ...
              case COMPLETION:
                logger.log(
                    LogLevel.Error,
                    "This client does not support %s messages",
                    message.getMessageType());

                throw new UnsupportedOperationException(
                    String.format(
                        "The message type %s is not supported yet.", message.getMessageType()));
            }
          }
        };
  ...
  public void start() throws Exception {
    ...
    transport.start();
    String handshake =
        HandshakeProtocol.createHandshakeRequestMessage(
  ...
  /**
   * Invokes a hub method on the server using the specified method name. Does not wait for a
   ...
   * @return A {@link Subscription} that can be disposed to unsubscribe from the hub method.
   */
  public <T1, T2> Subscription on(
      String target, Action2<T1, T2> callback, Class<T1> param1, Class<T2> param2) {
    ActionBase action =
        params -> {
   ...
   * @return A {@link Subscription} that can be disposed to unsubscribe from the hub method.
   */
  public <T1, T2, T3> Subscription on(
      String target,
   ...
   * @return A {@link Subscription} that can be disposed to unsubscribe from the hub method.
   */
  public <T1, T2, T3, T4> Subscription on(
      String target,
    ...
    ActionBase action =
        params -> {
          callback.invoke(
              param1.cast(params[0]),
              param2.cast(params[1]),
              param3.cast(params[2]),
              param4.cast(params[3]));
        };
    handlers.put(target, action);
    logger.log(LogLevel.Trace, "Registering handler for client method: %s", target);
    return new Subscription(handlers, action, target);
   ...
   * @return A {@link Subscription} that can be disposed to unsubscribe from the hub method.
   */
  public <T1, T2, T3, T4, T5> Subscription on(
      String target,
   ...
   * @return A {@link Subscription} that can be disposed to unsubscribe from the hub method.
   */
  public <T1, T2, T3, T4, T5, T6> Subscription on(
      String target,
   ...
   * @return A {@link Subscription} that can be disposed to unsubscribe from the hub method.
   */
  public <T1, T2, T3, T4, T5, T6, T7> Subscription on(
      String target,
   ...
   * @return A {@link Subscription} that can be disposed to unsubscribe from the hub method.
   */
  public <T1, T2, T3, T4, T5, T6, T7, T8> Subscription on(
      String target,
