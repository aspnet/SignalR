...
TEST(connection_token, connection_token_is_set_if_start_fails_but_negotiate_request_succeeds)
{
    std::shared_ptr<log_writer> writer(std::make_shared<memory_log_writer>());
    ...
    ASSERT_EQ(_XPLATSTR(""), connection->get_connection_token());
    start_task.get();
    ASSERT_EQ(_XPLATSTR("A=="), connection->get_connection_token());
}

TEST(connection_token, can_get_connection_token_when_connection_in_connected_state)
{
    auto writer = std::shared_ptr<log_writer>{ std::make_shared<memory_log_writer>() };
    ...
    connection->start()
        .then([connection, &connection_token]()
        mutable {
            connection_token = connection->get_connection_token();
            return connection->stop();
        }).get();
...
TEST(connection_token, can_get_connection_token_after_connection_has_stopped)
{
    auto writer = std::shared_ptr<log_writer>{ std::make_shared<memory_log_writer>() };
...
TEST(connection_token, connection_token_reset_when_starting_connection)
{
    auto fail_http_requests = false;
