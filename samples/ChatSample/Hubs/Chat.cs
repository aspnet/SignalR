...
namespace ChatSample.Hubs
{
    ...
    public class Chat : HubWithPresence
    {
        ...
        {
            await Clients.Client(Context.ConnectionId).Invoke("SetUsersOnline", await GetUsersOnline());
        ...
        {
            return Clients.Client(Context.ConnectionId).Invoke("UsersJoined", new[] { users });
        }

        public override Task OnUsersLeft(UserDetails[] users)
        {
            return Clients.Client(Context.ConnectionId).Invoke("UsersLeft", new[] { users });
        }
