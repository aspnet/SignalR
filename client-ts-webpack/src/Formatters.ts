...
let knownTypes: { [key: string]: MessageType } = {
    'T': MessageType.Text,
...
export namespace ServerSentEventsFormat {
    export function parse(input: string): Message {
        ...
        // Parse the header
        let messageType = knownTypes[header];
...
export namespace TextMessageFormat {
    ...
    function parseMessage(input: string, position: number): [number, Message] {
        ...
        // Read the length
        let [lenStr, offset2] = splitAt(input, ':', offset);
        ...
        if (!hasSpace(input, offset2, 3 + length)) {
            throw new Error('Message is incomplete');
        }

        // Read the type
        let [typeStr, offset3] = splitAt(input, ':', offset2);

        // Parse the type
        let messageType = knownTypes[typeStr];
        if (messageType === undefined) {
            throw new Error(`Unknown type value: '${typeStr}'`);
        }

        // Read the payload
        let payload = input.substr(offset3, length);
        offset3 += length;
        ...
        return [offset3, new Message(messageType, payload)];
    }
